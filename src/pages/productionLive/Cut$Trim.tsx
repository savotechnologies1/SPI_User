...
// // 1. Define types for the data structures
// export interface HourlyDataItem {
//   hour: string;
//   target?: number; // Made optional with '?'
//   actual: number;
//   scrap: number;
// }
...
//   total?: TotalData;
//   employees: (Employee | string)[]; // Allow objects or strings
// }
// // 2. Type the helper function
...
//   // 3. Type the accumulator in reduce
//   const shiftTotal = filteredData.reduce(
//     (acc, item) => {
//       acc.target += item.target || 0; // Handle optional target
//       acc.actual += item.actual;
//       acc.scrap += item.scrap;
//       return acc;
//     },
...
//               <img
//                 className="w-[20px]"
//                 src={emp.profileImage || (profile as string)}
//                 alt={emp.name}
//               />
//               <p className="text-xs">{emp.name}</p>
//             </div>
...
interface ProcessTableProps {
  ...
  hourlyData: HourlyDataItem[];
  total?: TotalData;
  employees: (Employee | string)[]; // Can be objects or just names as strings
}
